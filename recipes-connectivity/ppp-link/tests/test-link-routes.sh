...
run_monitor() {
    : > "$TMP/ip.log"
    : > "$TMP/ping.log"
    PATH="$TMP/bin:$PATH" IP_LOG="$TMP/ip.log" PING_LOG="$TMP/ping.log" \
        PING_RESULT="$1" PEER_CONFIG="$2" USB_PATH="$TMP/usb" LINK_ROUTE_ONCE=1 \
        "$ROOT/files/librescoot-link-route-monitor"
}

printf '%s\n' '192.168.8.1:192.168.8.2' > "$TMP/mdb-peer"
run_monitor success "$TMP/mdb-peer"
grep -Fqx -- '-c 1 -W 1 192.168.9.2' "$TMP/ping.log"
